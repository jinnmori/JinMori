...
function s.filter2(c,tp,sc)
	...
	return c:IsRace(RACE_FIEND) and c:IsType(TYPE_TUNER) and c:HasLevel() and c:IsAbleToGrave() and aux.SpElimFilter(c,true) 
		and aux.SelectUnselectGroup(rg,e,tp,nil,nil,s.rescon(c,sc),0)
end
function s.filter3(c)
	return c:HasLevel() and c:IsRace(RACE_FIEND) and not c:IsType(TYPE_TUNER) and c:IsAbleToGrave() and aux.SpElimFilter(c,true)
end
