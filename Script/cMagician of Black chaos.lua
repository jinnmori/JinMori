...
function s.contfilter(c)
	return c:IsCode(CARD_DARK_MAGICIAN_GIRL) and not c:IsDisabled()
end
function Ritual.Filter(c,filter,_type,e,tp,m,m2,forcedselection,specificmatfilter,lv,requirementfunc,sumpos)
	if not ((c:IsType(TYPE_RITUAL) or c:IsType(TYPE_FUSION)) and c:IsOriginalType(TYPE_MONSTER)) or (filter and not filter(c)) or not c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL|SUMMON_TYPE_FUSION,tp,true,true,sumpos) then return false end
...
function(filter,_type,lv,extrafil,extraop,matfilter,stage2,location,forcedselection,specificmatfilter,requirementfunc,sumpos,extratg)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,chk)
				if chk==0 then
					...
					return Duel.IsExistingMatchingCard(Ritual.Filter,tp,location,0,1,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos) or (Duel.IsExistingMatchingCard(Ritual.Filter,tp,location|LOCATION_EXTRA,0,1,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos)
						and Duel.IsExistingMatchingCard(s.contfilter,tp,LOCATION_ONFIELD,0,1,nil))
				end
...
function(filter,_type,lv,extrafil,extraop,matfilter,stage2,location,forcedselection,customoperation,specificmatfilter,requirementfunc,sumpos)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp)
				...
				if Duel.IsExistingMatchingCard(s.contfilter,tp,LOCATION_ONFIELD,0,1,nil) then 
					local tg1=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(Ritual.Filter),tp,LOCATION_EXTRA|location,0,1,1,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos)
					if #tg1>0 then
					local to=tg1:GetFirst()
					local lv=(lv and (type(lv)=="function" and lv(to)) or lv) or to:GetLevel()
					lv=math.max(1,lv)
					Ritual.SummoningLevel=lv
					local mat=nil
					mg:Match(Card.IsCanBeRitualMaterial,to,to)
					mg:Merge(mg2-to)
					if specificmatfilter then
						mg:Match(specificmatfilter,nil,to,mg,tp)
					end
					if to.ritual_custom_operation then
						to:ritual_custom_operation(mg,func,_type)
						mat=to:GetMaterial()
					else
						func=MergeForcedSelection(to.ritual_custom_check,func)
						if to.mat_filter then
							mg:Match(to.mat_filter,to,tp)
						end
						if ft>0 and not func then
							Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
							if _type==RITPROC_EQUAL then
								mat=mg:SelectWithSumEqual(tp,requirementfunc or Card.GetRitualLevel,lv,1,#mg,to)
							else
								mat=mg:SelectWithSumGreater(tp,requirementfunc or Card.GetRitualLevel,lv,to)
							end
						else
							mat=aux.SelectUnselectGroup(mg,e,tp,1,lv,Ritual.Check(to,lv,WrapTableReturn(func),_type,requirementfunc),1,tp,HINTMSG_RELEASE,Ritual.Finishcon(to,lv,requirementfunc,_type))
						end
					end
					...
					if not customoperation then
						to:SetMaterial(mat)
						if extraop then
							extraop(mat:Clone(),e,tp,eg,ep,ev,re,r,rp,to)
						else
							Duel.ReleaseRitualMaterial(mat)
						end
						Duel.BreakEffect()
						Duel.SpecialSummon(to,SUMMON_TYPE_RITUAL|SUMMON_TYPE_FUSION,tp,tp,true,true,sumpos)
						to:CompleteProcedure()
						if to:IsFacedown() then Duel.ConfirmCards(1-tp,to) end
						if stage2 then
							stage2(mat,e,tp,eg,ep,ev,re,r,rp,to)
						end
					else
						customoperation(mat:Clone(),e,tp,eg,ep,ev,re,r,rp,to)
					end
					Ritual.SummoningLevel=nil
				end
					else
				...
				if #tg>0 then
					...
					Ritual.SummoningLevel=nil
					Duel.RaiseEvent(tc,38033121,re,REASON_EFFECT,rp,ep,0)
			end
		end
	end
...
function s.val(e,c)
	return Duel.GetMatchingGroupCount(Card.IsCode,c:GetControler(),LOCATION_GRAVE,LOCATION_DECK+LOCATION_HAND,nil,30208479,CARD_DARK_MAGICIAN)*300
end
function s.attribute(e,c)
	return c:IsLevelAbove(6) and c:IsRace(RACE_SPELLCASTER) and c:IsType(TYPE_FUSION)
end