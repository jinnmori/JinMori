...
function Ritual.Filter(c,filter,_type,e,tp,m,m2,forcedselection,specificmatfilter,lv,requirementfunc,sumpos)
	if not ((c:IsOriginalType(TYPE_RITUAL) or (c:IsOriginalType(TYPE_FUSION) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,true,true,sumpos))) and c:IsOriginalType(TYPE_MONSTER)) or (filter and not filter(c)) or not c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,false,true,sumpos) then return false end
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,chk)
				if chk==0 then
					...
					if ExtraLocEff and ExtraLocEff:CheckCountLimit(tp) then
						...
						local ReqFunc=ExtraLocEff:GetTarget() or Card.IsMonster
						...
						local ExtraLocGroup=Duel.GetMatchingGroup(Ritual.Filter,tp,ExtraLoc,0,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos):Match(ReqFunc,nil,e:GetHandler())
...
function(filter,_type,lv,extrafil,extraop,matfilter,stage2,location,forcedselection,customoperation,specificmatfilter,requirementfunc,sumpos)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp)
				...
				if ExtraLocEff and ExtraLocEff:CheckCountLimit(tp) then
					...
					local ExtraLocGroup=Duel.GetMatchingGroup(aux.NecroValleyFilter(Ritual.Filter),tp,ExtraLoc,0,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos):Match(ReqFunc,nil,e:GetHandler())
					FinalGroup:Merge(BaseLocGroup)
					FinalGroup:Merge(ExtraLocGroup)
					tg=FinalGroup:Select(tp,1,1,nil)
				else
					tg=Duel.SelectMatchingCard(tp,aux.NecroValleyFilter(Ritual.Filter),tp,location,0,1,1,e:GetHandler(),filter,_type,e,tp,mg,mg2,func,specificmatfilter,lv,requirementfunc,sumpos)
				end
				if #tg>0 then
				...
				--------
					...
					if not customoperation then
						...
						Duel.SpecialSummon(tc,SUMMON_TYPE_FUSION,tp,tp,true,true,sumpos)
